...
const Posts = () => {
    ...
    return (
        <div>
            <Head><title>Blog Post - Sehmim</title></Head>
            <Navbar />

            <Container>
                {currentPosts && currentPosts.map((post) => {
                    return (
                        <PostEach key={post.id as Key} post={post} />
                    )
                })}
                ...
                <Flex padding={20} justifyContent={"center"}>
                    ...
                    <FlexEnd>
                        <Button onClick={() => setCurrentPage(currentPage + 1)} disabled={disableNext} >Next Page</Button>
                    </FlexEnd>
                </Flex>
            </Container>
        </div>
...
export async function getPosts(): Promise<Array<IPost>> {
    ...
    const rePosts = posts.filter((post: IPost | String) => post !== "Not found").sort((a: IPost, b: IPost) => {
